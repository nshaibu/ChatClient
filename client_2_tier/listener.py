...
class Message_Display_Popover(Gtk.Popover):
    ...
    def on_delete_msg_press(self, widget, event):
        try:
            self.widget_obj.users_manager.database_manager.cursor.execute(  "DELETE FROM %s.messages \
                                                                            WHERE message_id='%s'" % (
                                                                                users.__DB_NAME__,
                                                                                self.widget_obj.uid
                                                                            ))
    ...
    def on_msg_details_press(self, widget, event):
        try:
            self.widget_obj.users_manager.database_manager.cursor.execute(  "SELECT * FROM %s.messages \
                                                                            WHERE message_id='%s'" % ( 
                                                                                users.__DB_NAME__,
