...
def get_messages_from_db_to_display(builder, senderid, receiverid):
    ...
    try:
        users_manager.database_manager.cursor.execute("SELECT * FROM %s.messages \
                                                       WHERE sender_id=%d AND receiver_id=%d \
                                                       OR sender_id=%d AND receiver_id=%d \
                                                       ORDER BY time_received" % (users.__DB_NAME__, 
                                                       int(senderid), 
                                                       int(receiverid),
                                                       int(receiverid),
                                                       int(senderid) ) )
        ...
        for row in result:
            ...
            if int(row[4]) == -1:   #if not read
                try:
                    users_manager.database_manager.cursor.execute("UPDATE %s.messages SET time_read=%d \
                                                                    WHERE message_id='%s' " % (
                                                                        users.__DB_NAME__,
                                                                        time.time(),
                                                                        str(row[0])
...
def load_users_and_create_buttons(builder):
    ...
    users_manager.database_manager.cursor.execute("SELECT * FROM %s.users WHERE NOT userid = %d" %
                                                     (users.__DB_NAME__, users_manager.curr_sender_user))
...
class Main_window():
    ...
    def __init__(self):
        ...
        users_manager.database_manager.init_database_popover(self.builder)
        ...
        self.database_toolbutton.connect("clicked", users_manager.database_manager.popover_run)
...
class Message_Entry_Field():
    ...
    def __init__(self, builder, cli_socket):
        ...
        self.filechooser = files.File_Chooser(self.builder)
    ...
    def on_Messages_textentry_activate(self, widget):
        ...
        if self.client_socket.send_msg_dontwait( pk.packet_to_str() ):
            ...
            try:
                users_manager.database_manager.cursor.execute("INSERT INTO %s.messages \
                                                            VALUES('%s', %d, %d, %ld, %ld, %d, '%s') \
                                                            " % (users.__DB_NAME__,      #database name
